...
public:
    ...
    virtual bool colision(const sf::Sprite& jugador) const = 0; // funcion virtual puro para verificar colisiones
...
void reiniciarJuego(sf::Sprite& jugador, std::vector<Entidad*>& entidades, std::vector<bool>& carrilesOcupados, ZonaSegura& zonaSegura, sf::Texture& personajeTexture) {
    jugador.setPosition(anchoVentana / 2 - jugador.getGlobalBounds().width / 2, altoVentana - jugador.getGlobalBounds().height); // Posicion inicial del jugador
    ...
    zonaSegura.tamano.y = jugador.getGlobalBounds().height * 2.0f; // Aumentar el tamaño verticalmente
    ...
    jugador.setTexture(personajeTexture); // Establecer la textura del personaje
}
...
int main() {
    ...
    //Se crea jugador rectangulo sprite
    sf::Sprite jugador;
    sf::Texture personajeTexture; // Textura del personaje
    if (!personajeTexture.loadFromFile("apo2.png")) {
        std::cerr << "Error al cargar la textura del personaje" << std::endl;
        return 1; //ERROR
    }
    //Se establece la texture del jugador
    ...
    while (ventana.isOpen()) {
        ...
        if (jugador.getPosition().y < limiteReinicio) {
            reiniciarJuego(jugador, entidades, carrilesOcupados, zonaSegura, personajeTexture); // Reiniciar el juego si el jugador alcanza el limite de reinicio
        }
        ...
        eliminarCarrosFueraDePantalla(entidades); // Eliminar carros que estan fuera de la pantalla
