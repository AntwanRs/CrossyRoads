        ...
        while (window.isOpen()) { // Bucle principal
            ...
            while (window.pollEvent(evento)) {
                if (evento.type == sf::Event::Closed) {
                    // Cerrar la ventana si se presiona el boton de cerrar
                    window.close();
                }
                ...
                if (evento.type == sf::Event::KeyPressed && !seleccionado) {
                    ...
                    if (evento.key.code == sf::Keyboard::Left) {
                        ...
                        personajeSeleccionado = "Apo"; // Guardar el personaje seleccionado
                        seleccionado = true; // Marcar como seleccionado
                    }
                    else if (evento.key.code == sf::Keyboard::Right) {
                        text_ant.setFillColor(sf::Color::Blue);
                        personajeSeleccionado = "Ant"; // Guardar el personaje seleccionado
                        seleccionado = true; // Marcar como seleccionado
                    }
                    else if (evento.key.code == sf::Keyboard::Up) {
                        text_ale.setFillColor(sf::Color::Blue);
                        personajeSeleccionado = "Ale"; // Guardar el personaje seleccionado
                        seleccionado = true; // Marcar como seleccionado
                    }
                    else if (evento.key.code == sf::Keyboard::Down) {
                        text_alicia.setFillColor(sf::Color::Blue);
                        personajeSeleccionado = "Alicia"; // Guardar el personaje seleccionado
                        seleccionado = true; // Marcar como seleccionado
                    }
                }

                if (seleccionado && evento.type == sf::Event::KeyPressed && evento.key.code == sf::Keyboard::Space) {
                    window.close(); // Cerrar la ventana al presionar Enter
                    while (window.pollEvent(evento)) {} // Limpiar el buffer del teclado
                            ...
                            if (event.key.code == Keyboard::Return) {
                              ...
                              if (x == 1) {
                                  ...
                                  if (indicePersonajeSeleccionado != -1) {
                                      ...
                                      if (!texturaPersonaje.loadFromFile("personaje" + std::to_string(indicePersonajeSeleccionado + 1) + ".png")) {
                                          std::cerr << "Error al cargar la textura del personaje seleccionado" << std::endl;
                                          return -1;
                                      }
                                      // Asignar la textura del jugador al sprite del personaje
