...
int main() {
	...
	defaultTexture.loadFromFile("Texture/Crss.jpeg");
	...
	//cambio de imagen, esta es 1280
	defaultSprite.setScale(.75, .562);
	...
	Texture menuTexture;
	menuTexture.loadFromFile("Texture/Menu2.jpeg");
	background.setTexture(&menuTexture);

//cargamos musiquita
	//esto no ira aqui, planeo hacerle una libreria

	Music musiquita;
	if (!musiquita.openFromFile("Music/background.mp3"))
		cout << "no se pudo acceder a la cancion" << endl;
	musiquita.play();
